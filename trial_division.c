...
void run_demo()
{
    ...
    for (int i = 0; i < num_tests; i++)
    {
        ...
        if (years > 1e30)
            sprintf(est_str, "%.0e years", years);
        else if (years > 1e9)
            sprintf(est_str, "%.0f B years", years / 1e9);
        else if (years > 1e6)
            sprintf(est_str, "%.0f M years", years / 1e6);
        else if (years > 1000)
            sprintf(est_str, "%.0f K years", years / 1000);
        else if (years > 1)
            sprintf(est_str, "%.1f years", years);
        else if (est_seconds > 3600)
            sprintf(est_str, "%.1f hours", est_seconds / 3600);
        else if (est_seconds > 60)
            sprintf(est_str, "%.1f min", est_seconds / 60);
        else
            sprintf(est_str, "%.2f sec", est_seconds);
        
        printf("%-10d %15" PRIu64 " %10.4fs %15s\n", 
               tests[i].bits, iterations, time_spent, est_str);
